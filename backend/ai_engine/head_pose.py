...
def process_head_pose(frame, calibrated_angles=None):
    ...
    for face in faces:
        ...
        # Ensure calibrated_angles is properly assigned
        if calibrated_angles and len(calibrated_angles) == 3:
            pitch_offset, yaw_offset, roll_offset = calibrated_angles
        else:
            pitch_offset, yaw_offset, roll_offset = 0, 0, 0  # Default values if not calibrated
    ...
    while cap.isOpened():
        ...
        if not calibration_done:
            ...
            if elapsed_time >= CALIBRATION_TIME:
                print("Calibration Complete. Neutral angles:", calibrated_angles)
            ...
            cv2.putText(frame, f"Direction: {head_direction}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
